...
require("nvim-filetype").setup({
    ...
    -- 当为 true 时：首先显示配置的文件类型，然后显示所有其他类型
    -- 当为 false 时：仅显示配置的文件类型
    show_all_filetypes = true,
...
| 选项 | 类型 | 默认值 | 描述 |
|------|------|--------|------|
| `filetypes` | `string[]` | `{}` | 在选择器中优先显示的常用文件类型列表 |
| `show_all_filetypes` | `boolean` | `true` | 是否显示所有可用的文件类型或仅显示配置的类型 |
...
require("nvim-filetype").setup({
    ...
    show_all_filetypes = false,  -- 仅显示配置的文件类型
})
```
